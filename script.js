...
const tutorialGesture = document.getElementById('tutorial-gesture');
...
const tutorialSteps = [
    ...
    {
        title: "BE FAST!",
        desc: "The faster you finish, the more bonus coins you earn. Good luck!",
        gesture: "⏱️"
    }
];
...
const levels = [
    ...
    {
        , { x: 100, y: 460, w: 100, h: 20, type: 1 }, { x: 500, y: 20, w: 100, h: 20, type: 1 }
    },
    {
        , { x: 100, y: 460, w: 100, h: 20, type: 1 }, { x: 500, y: 20, w: 100, h: 20, type: 1 }
    },
    ...
    {
        , { x: 50, y: 20, w: 150, h: 20, type: 1 }, { x: 600, y: 460, w: 150, h: 20, type: 1 }
    },
    {
        , { x: 50, y: 20, w: 150, h: 20, type: 1 }, { x: 600, y: 460, w: 150, h: 20, type: 1 }
    },
...
function initLevel(index) {
    ...
    difficultyMultiplier = 1 + (index >= 4 ? (index * 0.15) : (index * 0.08)); // MUCH harder from level 5
    ...
    // Level 5+: Start adding hazards earlier
    if (index >= 4) {
        movingBlocks.push({ x: 150, y: 250, w: 40, h: 12, dx: 3.0 + index * 0.1, dy: 0 });
    }
    if (index >= 5) {
        movingBlocks.push({ x: 550, y: 150, w: 35, h: 12, dx: 0, dy: 2.5 + index * 0.1 });
    }
    if (index >= 6) {
...
function renderTutorialStep() {
    ...
    tutorialGesture.innerHTML = step.gesture;
    tutorialStepNum.innerText = currentTutorialStep + 1;
    
    tutorialProgressFill.style.width = `${((currentTutorialStep + 1) / tutorialSteps.length) * 100}%`;
    
    if (currentTutorialStep === tutorialSteps.length - 1) {
        tutorialNextBtn.innerHTML = "Play Game ▶️";
    } else {
...
function handleDeath(message) {
    ...
    sendPlayerData(document.getElementById('display-player-name').textContent, currentLevelIndex + 1, coins);
...
function handleCredentialResponse(response) {
    // Decoding JWT (simplified for the client-side profile access)
